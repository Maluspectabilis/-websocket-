...
public class WebSocketServer {
    ...
    * */
    ...
    public void onMessage(String message, @PathParam("username") String username) {
        // message:{"code":1, "to": "admin", "text": "聊天文本"}
        // message:{code:2,msg:图片信息,}
        ...
        if(code == 1){//文本信息
            ...
            if (toSession != null) {
                ...
                jsonObject.put("code",1);//表示转发消息为文本
            ...
            if (toSession != null) {
                ...
                JSONObject jsonObject = new JSONObject();
                jsonObject.put("from", username);  // from 是 zhang
                jsonObject.put("text",  obj.get("text"));  // text 同上面的text
                jsonObject.put("code",2);//表示转发消息为图片
                this.sendMessage(jsonObject.toString(), toSession);
                log.info("发送给用户username={}，图片消息：{}", toUsername, jsonObject.toString());
            } else {//离线发布
                log.info("图片发送失败，未找到用户username={}的session", toUsername);
            }
        }
